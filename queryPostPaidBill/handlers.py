...
def payment(data):
    ...
    with open(app_path+'/templates/payloads/payment.txt', 'r') as file:
        payment_template = file.read()
    payment_template = Template(changeSubOffering_template)
    payment = payment_template.substitute({**data.__dict__,"datetime":datetime.now().strftime("%Y-%m-%dT%H:%M:%S"),"C_FREE_PAY_FLAG" : 0 if data.payFlag==1 else 1})
    return AR_soap_client.call_service('payment', payment)
